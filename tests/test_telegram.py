...
async def test_task_error():
    ...
    await wait_bg(bot)
    ok("ошибка в чате", any("Не вышло: клод упал" in t for t in http.texts()))
...
async def test_voice():
    ...
    ok("распознанное показано", any("Услышал" in t and "открой ютуб" in t for t in http.texts()))
...
async def test_voice_edge():
    ...
    ok("тишина — «не расслышал», мозг не дёргался",
       any("Не расслышал" in t for t in http.texts()) and not bot._brain.asked)

    http2 = FakeHTTP()
    await bot._handle_update(http2, upd(15, 555, voice={"file_id": "v1", "file_size": 999_999_999}))
    await wait_bg(bot)
    ok("великан отбит без скачивания", any("слишком большое" in t for t in http2.texts())
...
async def test_model_cmd():
    ...
    ok("незнакомая модель отбита", cfg.model() == "opus"
       and any("Не знаю модель" in t for t in http.texts()))
...
async def test_screen_reset():
    ...
    try:
        ...
        tg_mod.brain.take_screenshot = orig
    ok("сбой скриншота — текст в чат", any("Скриншот не вышел" in t for t in http2.texts()))
    ...
    ok("незнакомая команда — подсказка", any("Не знаю такой команды" in t for t in http2.texts()))
...
async def test_stop():
    ...
    ok("нечего останавливать — честный ответ",
       any("Нечего останавливать" in t for t in http.texts()) and bot._brain.aborts == 1)

    bot._brain.abort_ret = True
    await bot._handle_update(http, upd(35, 555, "/stop"))
    ok("живой прогон — «останавливаю»", any("Останавливаю" in t for t in http.texts()))
    ...
    ok("в чате «Остановлено по вашей команде»",
       any("Остановлено по вашей команде" in t for t in http2.texts()))
...
async def test_restart_resilience():
    ...
    ok("прогресс помечен «прервано перезапуском»",
       any("прервано перезапуском" in t for t in http2.texts("editMessageText")),
    ...
    ok("хозяин уведомлён о прерванной задаче",
       any("прервана" in t for t in http3.texts()))
...
async def test_inflight_parallel():
    ...
    try:
        ...
        ok("во время STT /stop отвечает «останавливаю»",
           any("Останавливаю" in t for t in http.texts()), str(http.texts()))
